...
// Example:
//		ckm8cli query block --height=300
//		ckm8cli query block --hash=0xc88485a473527c55c5ddb067b018324b7e390b188e76702bc1db74dfc2dc6d13
//
var blockCmd = &cobra.Command{
	Use:     "block",
	Short:   "Get block details",
	Long:    `Get block details.`,
	Example: `ckm8cli query block --height=300`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		var err error
		if len(hashFlag) != 0 {
			res, err = client.Call("ckm8.GetBlock", rpc.GetBlockArgs{
				Hash:               common.HexToHash(hashFlag),
				IncludeEthTxHashes: includeEthTxHashFlag,
			})
		} else if endFlag != 0 {
			res, err = client.Call("ckm8.GetBlocksByRange", rpc.GetBlocksByRangeArgs{
				Start:              common.JSONUint64(startFlag),
				End:                common.JSONUint64(endFlag),
				IncludeEthTxHashes: includeEthTxHashFlag,
			})
		} else {
			res, err = client.Call("ckm8.GetBlockByHeight", rpc.GetBlockByHeightArgs{
