...
// Example:
//		ckm8cli query stake_returns --height=10
var stakeReturnsCmd = &cobra.Command{
	Use:     "stake_returns",
	Short:   "Get stake returns",
	Example: `ckm8cli query stake_returns, ckm8cli query stake_returns --height=800`,
...
func doStakeReturnsCmd(cmd *cobra.Command, args []string) {
	...
	var err error
	if height == 0 {
		res, err = client.Call("ckm8.GetAllPendingEliteEdgeNodeStakeReturns", rpc.GetAllPendingEliteEdgeNodeStakeReturnsArgs{})
	} else {
		res, err = client.Call("ckm8.GetEliteEdgeNodeStakeReturnsByHeight", rpc.GetEliteEdgeNodeStakeReturnsByHeightArgs{Height: common.JSONUint64(height)})
	}
	if err != nil {
