...
import (
	...
	"https://github.com/fsmile2/ckm8/dispatcher"
	"https://github.com/fsmile2/ckm8/rlp"
	"https://github.com/fsmile2/ckm8/store"
)
...
func (e *ConsensusEngine) validateBlock(block *core.Block, parent *core.ExtendedBlock) result.Result {
	...
	if block.GuardianVotes != nil && block.Height >= common.HeightEnableckm82 && common.IsCheckPointHeight(block.Height) {
	...
	if block.EliteEdgeNodeVotes != nil && block.Height >= common.HeightEnableckm83 && common.IsCheckPointHeight(block.Height) {
		// Voted block must exist.
		padding := uint64(20)
		if e.chain.Root().Height+padding*uint64(common.CheckpointInterval) < block.Height {
...
func (e *ConsensusEngine) createProposal(shouldIncludeValidatorUpdateTxs bool) (core.Proposal, error) {
	...
	// Add guardian votes.
	if block.Height >= common.HeightEnableckm82 && common.IsCheckPointHeight(block.Height) {
		block.GuardianVotes = e.guardian.GetBestVote()
	}

	// Add elite edge node votes.
	if block.Height >= common.HeightEnableckm83 && common.IsCheckPointHeight(block.Height) {
