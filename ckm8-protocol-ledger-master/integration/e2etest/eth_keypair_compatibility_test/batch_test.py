...
UNLOCK_KEY_CMD_TMPL = """curl -X POST -H 'Content-Type: application/json' --data '{"jsonrpc":"2.0","method":"ckm8cli.UnlockKey","params":[{"address":"%s", "password":"%s"}],"id":1}' http://localhost:16889/rpc"""
SEND_CMD_TMPL = """curl -X POST -H 'Content-Type: application/json' --data '{"jsonrpc":"2.0","method":"ckm8cli.Send","params":[{"chain_id":"testnet", "from":"%s", "to":"%s", "ckm8wei":"%s", "tfuelwei":"%s", "fee":"1000000000000", "sequence":"%s", "async":false}],"id":1}' --silent --output /dev/null http://localhost:16889/rpc"""
...
def GenerateNewKeystore():
  ...
  cp_cmd = 'cp %s/keystore/*--%s %s/keys/encrypted/%s'%(ETHEREUM_ROOT, address, ckm8CLI_ROOT, address)
  os.system(cp_cmd)
  
  return address

def GetInitialFaucetSeq():
  query_cmd = 'ckm8cli query account --address=%s'%(FAUCET_ADDRESS)
...
def BatchTest(init_faucet_seq):
  ...
  for idx in range(500):
    ...
    os.system("ckm8cli query account --address=%s"%(address))
    print("----------------------------------------------------------------")
    print("")
...
# __MAIN__
#
# Before running this script, we need to launch both the `ckm8` and `ckm8cli` daemon connected 
