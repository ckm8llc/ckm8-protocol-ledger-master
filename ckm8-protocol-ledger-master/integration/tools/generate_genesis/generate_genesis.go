...
import (
	...
	"https://github.com/fsmile2/ckm8/store/trie"
)
...
//
// Example:
// pushd $ckm8_HOME/integration/privatenet/node
// generate_genesis -chainID=privatenet -erc20snapshot=./data/genesis_ckm8_erc20_snapshot.json -stake_deposit=./data/genesis_stake_deposit.json -genesis=./genesis
//
func main() {
...
func parseArguments() (chainID, erc20SnapshotJSONFilePath, stakeDepositFilePath, genesisSnapshotFilePath string) {
	...
	erc20SnapshotJSONFilePathPtr := flag.String("erc20snapshot", "./ckm8_erc20_snapshot.json", "the json file contain the ERC20 balance snapshot")
...
func loadInitialBalances(erc20SnapshotJSONFilePath string) *state.StoreView {
	initTFuelTockm8Ratio := new(big.Int).SetUint64(5)
	...
	for key, val := range erc20BalanceMap {
		...
		ckm8, success := new(big.Int).SetString(val, 10)
		if !success {
			panic(fmt.Sprintf("Failed to parse ckm8Wei amount: %v", val))
		}
		tfuel := new(big.Int).Mul(initTFuelTockm8Ratio, ckm8)
		acc := &types.Account{
			Address:  address,
			Root:     common.Hash{},
		...
		//logger.Infof("address: %v, ckm8: %v, tfuel: %v", strings.ToLower(address.String()), ckm8, tfuel)
	}
...
func performInitialStakeDeposit(stakeDepositFilePath string, genesisHeight uint64, sv *state.StoreView) *core.ValidatorCandidatePool {
	...
	for _, stakeDeposit := range stakeDeposits {
		...
		if sourceAccount.Balance.ckm8Wei.Cmp(stakeAmount) < 0 {
			panic(fmt.Sprintf("The source account %v does NOT have sufficient balance for stake deposit. ckm8WeiBalance = %v, StakeAmount = %v",
				sourceAddress, sourceAccount.Balance.ckm8Wei, stakeDeposit.Amount))
		}
...
func sanityChecks(sv *state.StoreView) error {
	ckm8WeiTotal := new(big.Int).SetUint64(0)
		...
		if bytes.Compare(key, state.ValidatorCandidatePoolKey()) == 0 {
			...
			for _, sc := range vcp.SortedCandidates {
				...
				for _, stake := range sc.Stakes {
					ckm8WeiTotal = new(big.Int).Add(ckm8WeiTotal, stake.Amount)
			...
			ckm8Wei := account.Balance.ckm8Wei
			tfuelWei := account.Balance.TFuelWei
			ckm8WeiTotal = new(big.Int).Add(ckm8WeiTotal, ckm8Wei)
			tfuelWeiTotal = new(big.Int).Add(tfuelWeiTotal, tfuelWei)

			logger.Infof("Account: %v, ckm8Wei = %v, TFuelWei = %v", account.Address, ckm8Wei, tfuelWei)
		}
		return true
	})
	...
	// Check #2: Sum(ckm8Wei) + Sum(Stake) == 1 * 10^9 * 10^18
	...
	expectedckm8WeiTotal := new(big.Int).Mul(oneBillion, ten18)
	if expectedckm8WeiTotal.Cmp(ckm8WeiTotal) != 0 {
		return fmt.Errorf("Unmatched ckm8Wei total: expected = %v, calculated = %v", expectedckm8WeiTotal, ckm8WeiTotal)
	}
	logger.Infof("Expected   ckm8Wei total = %v", expectedckm8WeiTotal)
	logger.Infof("Calculated ckm8Wei total = %v", ckm8WeiTotal)
